...
import {API_WS, ebClearStorage, ebNavigateTo, ebSendSocketMessage, ebGetLocalStorage, ebSetLocalStorage, ebRequest,ebLogout} from '../utils'
			...
			try {
				switch (msg.message_type) {
					...
					case 'pushMessage'://聊天外推送
						try {
							let {push_type, push_content} = msg
							switch (push_type) {
								case 'pay_success':
									break;
								case 'logout_web':
									break;
								case 'logout_wx_min':
									dispatch(handleLogoutWxMin(push_content));
									break;
								case 'logout_app':
									break;
								case 'min_program':
 ...
 * @returns {Function}
 */
const handleLogoutWxMin = (push_content) => (dispatch, getState) => {
	...
	// if (push_content.token != account.token) {
	//   let content = '您已在:' + push_content.detail.address_login + ' 使用另一设备登录，本设备已自动下线。 ';
	//   ebShowModal({title: push_content.detail.title, content: content, showCancel: false, confirmText: '我知道了'}, () => {
	//     logoutWxMin()
	//   })
	// }
 ...
 * @param content 消息详情
 */
export const pushNotice = (type, id, push_type) => async (dispatch, getState) => {
	try {
		ebSendSocketMessage(JSON.stringify({type, id, push_type})); // push a message
	} catch (e) {
		console.error(e.stack || e);
		// alert(e + 'pushNotice ')
	}
