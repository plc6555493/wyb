...
import {
	...
	ebShowToast,
	ebGetUserInfo,
	ebSetLocalStorage, ENV, ebNavigateTo, ebSetColorState,
	ebGetLocalStorageInit, ebSetColorInit, ebRequest
} from '../../utils';
...
export default class Userlogin extends Component {
	...
	componentWillMount() {
		...
		let {bg_color, font_color} = ebSetColorState(this, 8);
		//console.log('usrLogin bg_color, font_color', bg_color, font_color)
	...
	componentWillReceiveProps(nextProps) {
		...
		self.setState({
			hideModel: !nextProps.showAuth && nextProps.scene!='1047'
		})
	}
	...
	getphone(e) {
		...
		if (e.currentTarget.encryptedData) {
			let session_key = ebGetLocalStorage('EB_SESSIONKEY');
	...
	render() {
		let {mobile, code, codeState, countdown, disabled, bg_color, top_info, hideModel} = this.state
		let {authState, sceneState, showAuth} = this.props

		let showManBind = false;

		//解决 h5 web 编译后  placeholder 被 encode
		let placeholder_phone = '请输入手机号码';

		let title = showManBind ? '请授权本人手机号' : '请授权微信绑定的手机号'

		let {init_info, style, color, border_bottom, search_bottom, show_search} = ebSetColorInit(top_info, 1);

		//console.log('bg_color, font_color',bg_color, font_color)
		...
		return (
			em ? null : showManBind ? <View className='body'>
							...
							<View className='view-main'>
								...
								<View className='entrance-input'>
									<Input
										...
										maxlength='11' style={{color: color, background: bg_color}}/>
								</View>
								{
								...
								<View className='pull_left'>
									...
									<View className='entrance-input-code'>
										<Input id='code'
													 ...
													 maxlength='6' style={{color: color, background: bg_color}}/>
				...
				<View className='user_getphone_mask'></View>
				<View className='user_getphone_view'>
					<View className='user_getphone_view_title' style={{color: color, background: bg_color}}>
						快速登录
					</View>
					...
					<View style='width:100%'>
						<Button openType='getPhoneNumber' onGetPhoneNumber={this.getphone} className='user_getphone_view_button pull-left' style={{color: color, background: bg_color}}>登录</Button>
