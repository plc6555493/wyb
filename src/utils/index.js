...
const ebRequest = (act, option, callBackSuccess, callbackFail) => {
  let { token, param, data, method, mock, mockIndex, showLoading } = option
  ...
  try {
    ...
    let url = (!mock ? API : mockApi[mockIndex]) + act + param
...
export const _uploadImageH5 = (options, callback, token) => async (dispatch, getState) => {
  console.log('_uploadImageH5 getState', getState)
...
const ebSetColorTo = (from) => {
  let { bgColor: backgroundColor, fontColor: frontColor } = ebSetColor('1' + from)
  // console.log('setColor bgColor, fontColor', backgroundColor, frontColor)
...
//   Taro.getSystemInfo({
//     success: (res) => {
//       return successCallBack ? successCallBack(res) : 750 * res.windowHeight / res.windowWidth
//     }
//   })
// }
 ...
 *
 * @param from
 * @returns {{bgColor: string, fontColor: string}}
 */
const ebSetColor = (from = 'default') => {
  ...
  let bgColor = init ? ENV === 'H5' ? '#' + init['color'] : init['color'] : '#3176e7'
  let fontColor = bgColor === '#ffffff' ? '#000000' : '#ffffff'
...
const ebSetColorState = (that, from = 'default') => {
  ...
  let { bgColor, fontColor } = ebSetColor(from)
  that.setState({
    bgColor,
    fontColor
  })
 ...
 * @param from
 */
const ebSetColorInit = (topInfo = {}, from = 1) => {
  let initInfo = ebGetLocalStorageInit()
  let initColor = initInfo['color']

  if (initInfo === null || JSON.stringify(initInfo) === '{}') {
    initInfo = {}
    initColor = '#ffffff'
  } else {
    // revert replaced '#'
  ...
  let showSearch = topInfo && topInfo.status ? 'none' : 'block'
  ...
  if (initColor === '#ffffff') {
    ...
    searchBottom = 'color:#666666'
    color = '#666666'
  } else {
    style += ';color:#ffffff;'
    borderBottom = 'border-bottom:1px solid #ffffff'
    searchBottom = 'color:#ffffff'
  }

  switch (from) {
    case 1:
      // search use
      searchBottom = searchBottom += ';font-size:14px'
      return {
        initInfo,
        style,
        color,
...
const ebSetNavigationBarColor = (frontColor, backgroundColor) => {
  if (ENV === 'WEAPP') {
    Taro.setNavigationBarColor({
      ...
      backgroundColor: backgroundColor || frontColor,
      success: function () {
        ...
        success: function (res) {
          ...
          ebRequest('/wxmin/' + API_V1 + 'User/getToken', option, (result) => {
            if (result.status) {
              ...
              ebSetLocalStorage('EB_SESSIONKEY', result['data']['sessionKey'])
...
const ebGetAuthState = () => {
  return !!ebGetLocalStorage('EB_TOKEN')
}

const ebLogout = () => {
  Taro.getApp().props.authorizedUpdate(false)
  ebClearStorage()
  ebReLaunch()
}
...
const ebPay = (oid, type = 1) => {
  ...
  ebRequest('/wxmin/' + API_V1 + 'pay', option, function (result) {
    if (result.status) {
      console.log('支付接口调用成功')
      let { timeStamp, nonceStr, signType, paySign, appId } = result.data
      // 支付
      Taro.requestPayment({
        appId,
        timeStamp,
...
const ebFilePreview = (option) => {
  ...
  console.log('ebFilePreview ext:', ext)

  if (ext === 'doc' || ext === 'docx' || ext === 'xls' || ext === 'xlsx' || ext === 'ppt' || ext === 'pptx' || ext === 'pdf') {
    console.log('ebFilePreview todo')

    if (SystemInfoUtil.platform === SystemInfoUtil.IOS || SystemInfoUtil.platform === SystemInfoUtil.PC) {
      console.log('ebFilePreview ios pc')

      let ebsrc = ext === 'pdf' ? previewSrc : 'https://web.ewyb.cn/preview?t0=pdf&u0=' + previewSrc

      url = ext === 'pdf' ? '/com/webView/index?ebsrc=' + previewSrc : '/com/webView/index?ebsrc=' + ebsrc

      ebNavigateTo(url)
    } else if (SystemInfoUtil.platform === SystemInfoUtil.ANDROID) {
      console.log('ebFilePreview android')
...
/**
 * 订单状态：orderState 0:已取消;10:未付款(默认);20:已付款;30:待打印;40:已完成 (已打印);50:已完成(退款的);60已删除
 * 可用 order_state 0, 60
 * @param options
 */
const orderPrintUpdate = (options, successCallback) => {
  let { orderState, oid } = options
  if (orderState === undefined) {
    ebShowToast('状态不能为空', 5000)
  } else {
    let token = ebGetLocalStorage('EB_TOKEN')
    let data = { order_state: orderState, oid }
    let option = { data, token, method: 'POST' }
